...
희소 테이블 문제.
int 범위로 해결 가능하다.
막막한 부분은, 한 굴에서 갈 수 있는 굴이 다양하다는 점이다. 어디쪽이 1에
가까운지 알 수 없다. n-1이라는 것으로 트리라는 것을 알 수 있다. 방향을 알기
위해서 dfs부터 하고 방향을 정한 뒤에, 원래 희소 테이블을 활용해주면 된다.
20200416
*/
#include <cstdio>
#include <iostream>
...
void dfs(int start) {
  ...
  for (int i = 0; i < adj[start].size(); i++) {
    ...
    if (isChecked[end] == 0) {
      denDirection[end][0] = mp(start, energy);
      dfs(end);
    }
  }
}
...
int main() {
  ...
  for (int i = 1; i <= 19; i++)
    for (int j = 1; j <= n; j++) {
      denDirection[j][i].first =
          denDirection[denDirection[j][i - 1].first][i - 1].first;
      denDirection[j][i].second =
          denDirection[denDirection[j][i - 1].first][i - 1].second +
          denDirection[j][i - 1].second;
    }
  for (int i = 1; i <= n; i++) {
    int now = i;
